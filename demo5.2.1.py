...
collection_name = "luat_lao_dong_vietnam"
...
def retrieve_from_pdf(query: str, n_results: int = 4) -> str:
    """
    Tìm kiếm trong cơ sở dữ liệu PDF Luật Lao động Việt Nam và trả về các đoạn văn liên quan nhất.
...
SYSTEM_PROMPT = """Bạn là trợ lý AI có quyền truy cập cơ sở dữ liệu PDF về Luật Lao động Việt Nam.
...
if __name__ == "__main__":
    print("\nRAG Agent ready.")
    print("Commands:")
    print("  ingest <path/to/file.pdf>  — load a PDF or folder into the knowledge base")
    print("  quit                       — exit")
    ...
    while True:
        ...
        if user_input.lower().startswith("ingest "):
            folder_path = user_input[7:].strip()
            if os.path.isdir(folder_path):
                # Lấy danh sách tất cả file .pdf trong thư mục
                pdf_files = glob.glob(os.path.join(folder_path, "*.pdf"))
                print(f"🔍 Tìm thấy {len(pdf_files)} file. Đang tiến hành nạp lần lượt...")
                
                # Vòng lặp nạp từng file bằng hàm ingest_pdf có sẵn của bạn
                for pdf in pdf_files:
                    print(f"Đang xử lý: {pdf}")
                    ingest_pdf(pdf) 
                    
                print("✅ Đã nạp xong toàn bộ thư mục!")
            else:
